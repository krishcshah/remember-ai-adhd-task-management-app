...
export const RepeatModal: React.FC = () => {
  ...
  if (!isRepeatOpen || !repeatTargetTask) return null;
  ...
  const handleSave = () => {
    let finalType = repeatType;
    let finalDays = selectedDays;

    if (finalType === 'weekly_on' && finalDays.length === 0) {
      // If user selected weekly on but didn't pick days, default to current day
    ...
    updateTask(repeatTargetTask.id, {
      repeatType: finalType,
      repeatDays: finalType === 'weekly_on' ? finalDays : undefined,
      repeatDaily: isDaily,
    });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white dark:bg-stone-900 w-full max-w-sm rounded-t-3xl sm:rounded-3xl p-5 shadow-2xl border border-stone-200/80 dark:border-stone-800 animate-slideUp">
        {/* Header */}
              ...
              {DAYS_OF_WEEK.map((d) => {
                ...
                return (
                  <button
                    ...
                    className={`h-9 rounded-xl text-xs font-bold transition-all flex flex-col items-center justify-center ${
                      isDaySelected
